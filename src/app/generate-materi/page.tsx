...
export default function GenerateMateri() {
    ...
    return (
        <div className="relative flex pt-32 pb-32 w-full flex-col items-center justify-center overflow-hidden text-white">
            ...
            </button>
            {
                ...
                <>
                    <div className="text-center font-urbanis mb-6">
                        <p className="text-2xl mb-3">Menampilkan materi untuk {formData.materi_apa}</p>
                    ...
                    <div className="bg-white w-full pt-16 pb-24 font-urbanis rounded-t-[100px] relative z-10">
                        {
                            !isBookmarked &&
                            <div onClick={handleBookmark} className="bg-white shadow-lg p-2 rounded-xl absolute cursor-pointer -top-6 left-28">
                                <BookmarkIcon />
                            </div>
                        }
                        <Markdown className="text-start text-[#33313B] mx-auto prose lg:prose-xl">{result}</Markdown>
                        <button onClick={handleRegenerateMateri} className="absolute -bottom-8 left-0 right-0 w-max text-lg py-5 px-12 rounded-full bg-gradient-to-r from-[#99A6D5] to-[#5B75FE] mx-auto block font-bold shadow-lg shadow-[#EC27B6] shadow-button">Belajar Lainnya</button>
                    </div>
                </>
                :
                <form onSubmit={handleGenerateMateri} className="font-urbanis relative z-10 container">
                    <div className="flex flex-col mb-10">
                        ...
                        <input type="text" id="belajar_apa" name='materi_apa' value={formData.materi_apa} onChange={(e) => setFormData({...formData, materi_apa: e.target.value})} className="bg-[#D9D9D9] rounded-2xl p-4 text-black font-medium text-lg duration-200 focus:outline-2 focus:outline-purple-600" required />
                    </div>
                    <div className="flex mb-12">
                        <div className="flex flex-col flex-1 mr-16">
                            ...
                            <select id="tingkatan" name='tingkat' value={formData.tingkat} onChange={(e) => setFormData({...formData, tingkat: e.target.value})} className="bg-[#D9D9D9] rounded-2xl p-4 text-black font-medium text-lg duration-200 focus:outline-2 focus:outline-purple-600">
                        ...
                        <div className="flex flex-col flex-1">
                            ...
                            <select id="berapa_lama" name='durasi_belajar' value={formData.durasi_belajar} onChange={(e) => setFormData({...formData, durasi_belajar: e.target.value})} className="bg-[#D9D9D9] rounded-2xl p-4 text-black font-medium text-lg duration-200 focus:outline-2 focus:outline-purple-600">
                                <option value="Seminggu">Seminggu</option>
                                <option value="Satu bulan">Satu Bulan</option>
                                <option value="Lebih dari satu bulan">Lebih dari satu bulan</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col mb-14">
                        ...
                        <textarea id="deskripsiin" name='desc_belajar' rows={10} value={formData.desc_belajar} onChange={(e) => setFormData({...formData, desc_belajar: e.target.value})} className="bg-[#D9D9D9] rounded-2xl p-4 text-black font-medium text-lg duration-200 focus:outline-2 focus:outline-purple-600" required></textarea>
                    </div>
